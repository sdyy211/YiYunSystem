//
//  KQChaiTimeViewController.swift
//  YinYunSystem
//
...
class KQChaiTimeViewController: UIViewController {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "UnwindAskForChaiDone" {
            
            
            let begin = datefor.stringFromDate(beginTime.date)
            let end = datefor.stringFromDate(endTime.date)
            let viewController = segue.destinationViewController as! KQChaiTableViewController
        ...
        } else if segue.identifier == "UnwindAskForChai" {
            if begin != nil {
