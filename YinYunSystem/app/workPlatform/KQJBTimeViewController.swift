...
class KQJBTimeViewController: UIViewController {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "UnwindAskForJBDone" {
            
            
            let begin = datefor.stringFromDate(beginTime.date)
            let end = datefor.stringFromDate(endTime.date)
            let viewController = segue.destinationViewController as! KQJBTableViewController
        ...
        } else if segue.identifier == "UnwindAskForJB" {
            if begin != nil {
